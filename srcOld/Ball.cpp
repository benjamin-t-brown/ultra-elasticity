...
Ball::Ball(Game& gameA)
    : Actor(gameA, "invisible"),
      ...
      speed(1.0),
      isSticky(false) {
  anims["ball_normal"] = game.window.getStore().createAnimation("ball_normal");
  anims["ball_armored"] = game.window.getStore().createAnimation("ball_armored");
  anims["ball_extra"] = game.window.getStore().createAnimation("ball_extra");
...
void Ball::update() {
  ...
  double frameRatio = game.window.getDeltaTime() / (1000.0 / 60.0);
  ...
  auto [w, h] = game.window.getRenderDims();
  if (y > h - 8) {
    remove();
  }
}
...
void Ball::draw() {
  ...
  if (!isSticky) {
    for (unsigned int i = 0; i < previousStates.size(); i++) {
      ...
      d.drawAnimation(
          anim,
          sdl2w::RenderableParams{.x = static_cast<int>(state.first),
                                  .y = static_cast<int>(state.second)});
